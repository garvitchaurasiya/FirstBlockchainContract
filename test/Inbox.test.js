...
describe('Inbox', ()=>{
    ...
    it('has a default message', async ()=>{
        const message = await inbox.methods.message().call(); // Remember we have made the message variable public so solidity by defualt create a method of same name as variable which can be called to access the value of that variable.
        assert.equal(message, 'Hi there!');
    })

    it('can change the message', async()=>{
        await inbox.methods.setMessage('Bye').send( {from: accounts[0]} ); //We have to send the account address which account is paying the gas for modifing the data. We can think of send right here as being like send transcation. This right here will return a transaction hash or we can say a reciept why which we can assume that the transaction is succeful, we don't really have to store the transaction hash if it doen't return the hash a error will be thrown and our test fails.
        const message = await inbox.methods.message().call(); // Remember we have made the message variable public so solidity by defualt create a method of same name as variable which can be called to access the value of that variable.
